...
contains
 ...
 !********************************************************************
  ...
  !!##############################################################################
   ...
   box_size=(/ -centr(1),ncm_tot(1)*gstep(1)-centr(1),&
              -centr(2),ncm_tot(2)*gstep(2)-centr(2),&
              -centr(3),ncm_tot(3)*gstep(3)-centr(3) /)
...
!!############################################################
   ...
   !!           - Denw
    ...
    write(write_name,'(a3,a1,a,a1,a,a,a,a)')trim(prefix),"_",&
         trim(run_name(1:len_trim(run_name)-7)),"_",&
         trim(traj_name(1:len_trim(traj_name)-4)),"_",&
         trim(adjustl(msg_angle)),".xml"
    ...
    call get_VOTIME(YYYY(i),MM(i),DD(i),hh(i),minut(i),ss(i),votime)
          write(iunit,'(2a,4(a,e12.3),a)') '<TD>',votime,'</TD> <TD>',A_1D(i),'</TD> <TD>',&
               Ax_1D(i),'</TD> <TD>',Ay_1D(i),'</TD> <TD>',Az_1D(i),'</TD>'
          write(iunit,'(a)') '</TR>'    
    enddo
  ! finalize the file
...
!    close(iunit)
    ...
    vect_norm=sqrt(Ax_1D(i)**2+Ax_1D(i)**2+Ax_1D(i)**2)
          write(iunit,'(2a,6(a,e12.3),a)') '<TD>',votime,'</TD> <TD>',A_1D(i),'</TD><TD>',&
               vect_norm,'</TD><TD>',Ax_1D(i),'</TD> <TD>',Ay_1D(i),'</TD> <TD>',&
               Az_1D(i),'</TD> <TD>',B_1D(i),'</TD>'
          write(iunit,'(a)') '</TR>'    
    enddo
  ! finalize the file
  write(iunit,'(a)') '</TABLEDATA>'
